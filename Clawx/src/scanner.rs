...
impl<'a> Scanner<'a> {
    ...
    fn number(&mut self) -> Token<'a> {
        ...
        if self.peek() == Some(&b'.') && self 
            .peek_next().map(|c| c.is_ascii_digit()).unwrap_or(false) {
                self.advance();
                while self.peek().map(|c| c.is_ascii_digit()).unwrap_or(false){
                    self.advance();
                }
    ...
    fn identifier(&mut self) -> Token<'a> {
        while self.peek().map(|c| c.is_ascii_alphanumeric() || c == &b'_')
            .unwrap_or(false){
                self.advance();
            }

            let token_kind = self.identifer_type();
            self.make_token(token_kind)
    }
    
    fn identifer_type(&mut self) -> TokenKind {
